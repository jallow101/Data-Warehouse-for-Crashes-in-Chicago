...
def clean_vehicle_data_exact(rows):
   ...
   # Step 4: Remove rows with any missing values but keep VEHICLE_YEAR as int
    ...
    for row in rows: #filtered_rows:
        ...
        if not row['VEHICLE_YEAR'] or row['VEHICLE_YEAR'] == "":
            
            row['VEHICLE_YEAR'] = 0
            #Assign VEHICLE_ID based on UNIT_TYPE if VEHICLE_ID is empty
        ...
        if row['VEHICLE_ID'] == "Delete ME":
            print("Excluding -->", row['UNIT_TYPE'], " ---", row['VEHICLE_ID'])
        else:
